...
aspmFormReadOS <- function (form, omit.missing = FALSE, constrasts = NULL)
{
  ...
  if (rhs[1] != "1") {
    for (i in 1:length(rhs)) {
      term <- rhs[i]
      type <- SemiPar::spmTermType(term)
      if (type == "offset")
        off.set <- eval(parse(text = term))
      if (type == "lin") {
        if (is.factor(eval(parse(text = term)))) {
          temp = stats::model.matrix(stats::as.formula(paste("~", term)),
                              constrasts = stats::contrasts)
          lin$contrasts <- c(lin$contrasts, attr(temp,
                                                 "contrasts")[[term]])
    ...
    if (length(pen$name) == 1) {
      if (pen$adf[[1]] == "miss") {
        ...
        arg.list <- substring(term, 3, (nchar(term) -
                                          1))
        out <- SemiPar::arg.search(arg.list, "df=")
        present <- out$present
        arg <- out$arg
        if (present == FALSE)
          pen$adf[[1]] <- "miss"
        if (present == TRUE)
          pen$adf[[1]] <- SemiPar::spmArgRead(arg)$val - 1
      }
    }
  }
  if (is.null(lin$x))
  ...
  if (!is.null(spm.info$pen$basis))
    for (j in 1:length(spm.info$pen$degree)) if (spm.info$pen$basis ==
                                                 "os" & length(spm.info$pen$degree[[j]]) == 1) {
      if (((spm.info$pen$degree[[j]] + 1)/2)%%1 != 0) {
        ...
        spm.info$pen$degree[[j]] = c(3, 2)
      }
      else spm.info$pen$degree[[j]] <- c(spm.info$pen$degree[[j]],
                                         (spm.info$pen$degree[[j]] + 1)/2)
    }
  return(spm.info)
}
