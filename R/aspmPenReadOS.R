...
aspmPenReadOS <- function (term)
{
  arg.list <- substring(term, 3, (nchar(term) - 1))
  var.name <- SemiPar::break.string(arg.list, ",")[1]
  ...
  if (present == FALSE)
    var.knots <- NULL
  if (present == TRUE)
    var.knots <- spmArgReadOS(arg)$val
  ...
  if (present == FALSE)
    var.basis <- basis
  if (present == TRUE)
    var.basis <- spmArgReadOS(arg)$val
  ...
  if (present == FALSE)
    var.degree <- degree
  if (present == TRUE)
    var.degree <- spmArgReadOS(arg)$val
  return(list(name = var.name, var = var.val, knots = knots,
              adap = adap, var.knots = var.knots, var.basis = var.basis,
              var.degree = var.degree, spar = spar, adf = adf, degree = degree,
              basis = basis))
}
