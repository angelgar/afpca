...
The full simulations in the paper used 100 replications per scenario and
three sample sizes $`I \in \{25, 50, 100\}`$; this vignette uses 10
replications and two sample sizes $`I \in \{15, 25\}`$ to keep runtime
manageable, as the original simulations took around 8+ hours to run
using 10 cores and all intermediate output was saved to monitor
convergence, resulting in 2GB+ of data. The conclusions remain the same.
